...
const BusinessForm = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center'>
        <div className='flex w-96 justify-between items-center p-3 m-3'>
            <div className='flex flex-col justify-center items-center'>
                <h3 className= {page === 1 || page === 2 || page === 3 || page === 4 ? 'inline-block bg-blue-600 py-1 px-3 rounded-full text-white' : 'bg-slate-200 py-2 px-4 rounded-full'}>1</h3>
      ...
      {page === 2 && (
        <form onSubmit={handleNext}  className="flex flex-col gap-4 w-full m-8 rounded-2xl">
          ...
          <label>
            <input type="text" required placeholder="Country" name="country" className='border-b border-slate-300 p-3 w-full outline-none' value={formData.country ? `${formData.country}`: ""} onChange={handleChange} />
          </label>
          <label>
            <input type="text" required placeholder="Currency" name="currency" className='border-b border-slate-300 p-3 w-full outline-none' value={formData.currency ? `${formData.currency}`: ""} onChange={handleChange} />
          </label>
      ...
      {
        page === 4 && (
            <SubmitPage data={formData} type = {2}/>
        ) 
      }
    </div>
  )
